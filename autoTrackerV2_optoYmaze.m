...
stimFreq=5;                                     % Frequency of stimulus pulse in hz
...
while toc < exp_duration
        ...
        if pixDev(mod(ct,10)+1)<8
            ...
            choiceArm(mazeOri)=choiceArm(mazeOri)+3;        % Adjust maze arm index by 3, rightside up Ys
            
            % Select appropriate arm coordinates for stimulation
            [stimCoords,activeStim,stim_tStamps]=...
                getStimTargets(arm_coords,choiceArm,changedArm,activeStim,occupyingLitArm,tElapsed);
            
            % Convert stim coordinates from camera to projector coords
            
            % Display new stimuli
            [pulseON,stim_tStamps]=optoDispCheckerStim(stimProps,stimCoords,pulseON,activeStim,stim_tStamps,stimFreq,tElapsed);
